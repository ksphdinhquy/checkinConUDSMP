...
const DiemDanh = () => {
    ...
    const handleConfirm = async (user) => {
        const date = Date.now(); // Lấy timestamp hiện tại

        const checkInQuery = query(
            collection(db, "diemdanh"),
            where("hovaten", "==", user.hovaten),
            where("checkIn", ">=", moment().startOf('day').valueOf()), // Check for check-ins on the same day
            where("checkIn", "<=", moment().endOf('day').valueOf())
        );
        ...
        if (existingCheckIns.size > 0) {
            ...
            await setDoc(doc(db, "diemdanh", uuidv4()), {
                ...user,
                checkIn: date, // Lưu timestamp của lúc điểm danh
            });
